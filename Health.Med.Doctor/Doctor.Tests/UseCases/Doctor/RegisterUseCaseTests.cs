...
public class RegisterUseCaseTests
{
    ...
    public async Task RegisterDoctorAsync_ShouldReturnValidationError_WhenEmailIsAlreadyRegistered()
    {
        // Arrange
        var request = new RequestRegisterDoctor("Doctor Name", "Name", "email@example.com", "12345", "password", Guid.NewGuid(), 0.0m);
        var existingDoctor = new Domain.Entities.Doctor(Guid.NewGuid(), DateTime.UtcNow, "Doctor Name", "Name", request.Email, "12345", "password", Guid.NewGuid(), 0.0m);
    ...
    public async Task RegisterDoctorAsync_ShouldReturnValidationError_WhenCRIsAlreadyRegistered()
    {
        // Arrange
        var request = new RequestRegisterDoctor("Doctor Name", "Name", "email@example.com", "12345", "password", Guid.NewGuid(), 0.0m);
        var existingDoctor = new Domain.Entities.Doctor(Guid.NewGuid(), DateTime.UtcNow, "Doctor Name", "Name", "otheremail@example.com", request.CR, "password", Guid.NewGuid(), 0.0m);
