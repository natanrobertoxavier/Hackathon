    ...
    IHttpContextAccessor httpContextAccessor) : Base, IDoctorServiceApi
{
    ...
    public async Task<Result<DoctorResult>> RecoverByIdAsync(Guid doctorId)
    {
        _logger.Information($"{nameof(RecoverByIdAsync)} - Iniciando a chamada para API de médicos. Médico: {doctorId}.");
        ...
        try
        {
            ...
            var uri = string.Format("/api/v1/doctor/id/{0}", doctorId);
                ...
                response.StatusCode is System.Net.HttpStatusCode.NotFound)
            {
                ...
                _logger.Information($"{nameof(RecoverByIdAsync)} - Encerrando chamada para API de médicos. Médico: {doctorId}.");

                return responseApi;
            }

            var failMessage = $"{nameof(RecoverByIdAsync)} - Ocorreu um erro ao chamar a API de médicos. StatusCode: {response.StatusCode}";
        ...
        {
            var errorMessage = $"{nameof(RecoverByIdAsync)} - Ocorreu um erro ao chamar a API de médicos. Erro: {ex.Message}";
